<script type="text/javascript">
    /**
     * Funciones javascript para validar los campos de los formularios de las vistas asignacionQA
    ...
    function comprobarAlfabetico(campo, size){
        ...
        if(!letras.test(campo.value)){
            ...
            return false;
        }   //Si cumple la expresion regular se comprueba el tamaño
    ...
    function comprobarAsigADD(){


        var idTrabajo; //obtenemos input IdTrabajo del formulario
        var loginEvaluador; //obtenemos input loginEvaluador del formulario
        var loginEvaluado; //obtenemos input loginEvaluado del formulario
        var aliasEvaluado; //obtenemos input aliasEvaluado del formulario
        var toret; //devuelve true o false en funcion de si los campos del formulario son correctos


        idTrabajo = document.getElementById("idTrabajo1");
        loginEvaluador = document.getElementById("loginEvaluador1");
        loginEvaluado = document.getElementById("loginEvaluado1");
        aliasEvaluado = document.getElementById("aliasEvaluado1");
        toret =true; //inicializamos a true

        //comprobamos los inputs con sus respectivas funciones de comprobaciones 
        if(!(comprobarVacio(idTrabajo) && comprobarEspacio(idTrabajo) && comprobarTexto(idTrabajo,6))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarVacio(loginEvaluador) && comprobarEspacio(loginEvaluador) && comprobarTexto(loginEvaluador,9))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarVacio(loginEvaluado) && comprobarEspacio(loginEvaluado) && comprobarTexto(loginEvaluado,9))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarVacio(aliasEvaluado) && comprobarEspacio(aliasEvaluado) && comprobarTexto(aliasEvaluado,6))){
            toret = false;
    ...
    function comprobarAsigSEARCH(){
        ...
        //Obtenemos los inputs del formulario
        idTrabajo = document.getElementById("idTrabajo");
        loginEvaluador = document.getElementById("loginEvaluador");
        loginEvaluado = document.getElementById("loginEvaluado");
        aliasEvaluado = document.getElementById("aliasEvaluado");
        toret =true;

        //comprobamos los inputs con sus respectivas funciones de comprobaciones 
        if(!(comprobarstartEspacio(idTrabajo) && comprobarTexto(idTrabajo,6))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarstartEspacio(loginEvaluador) && comprobarTexto(loginEvaluador,9))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarstartEspacio(loginEvaluado) && comprobarTexto(loginEvaluado,9))){
            toret = false;
            
        }

        //comprobamos los inputs con sus respectivas funciones de comprobaciones
        if(!(comprobarstartEspacio(aliasEvaluado) && comprobarTexto(aliasEvaluado,6))){
            toret = false;
